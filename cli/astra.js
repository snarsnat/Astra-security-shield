...
function wireSDK(projectDir, projectName, appToken) {
  ...
  if (fs.existsSync(pkgPath)) {
    ...
    // Add astra-shield to dependencies if not present
  ...
  // Write env var to .env.local (gitignored)
  ...
  // Write astra-init.js to project root
  ...
  const importLine = framework === 'vite' || framework === 'nuxt'
    ? `import { ASTRAShield } from 'astra-shield';`
    : `import { ASTRAShield } from 'astra-shield';`;

  const initCode = `// Astra Shield — auto-generated by \`astra add\`
// Protects ${projectName} with behavioral bot detection & human verification.
// Docs: https://astra-shield-site.vercel.app
...
shield.on('challenge', (d) => console.log('[Astra] Challenge:', d.type));
shield.on('success',   (d) => console.log('[Astra] Verified — tier:', d.tier));
shield.on('blocked',   (d) => console.log('[Astra] Blocked:', d.reason));

// Usage: import { shield } from './astra-init.js'
// Protect an action: const result = await shield.protect('checkout', { item: 'hoodie' })
// If result.success — proceed. If !result.success — block.
`;

  const initPath = path.join(projectDir, 'astra-init.js');
  if (!fs.existsSync(initPath)) {
    fs.writeFileSync(initPath, initCode);
    ok(`Created astra-init.js (import shield from here)`);
  } else {
    warn(`astra-init.js already exists — skipping (update appToken manually if needed)`);
  }
}
